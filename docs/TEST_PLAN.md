...
What the integration and end-to-end suites check, and why each check
exists, written for someone who has never seen the codebase: each case says
what situation we set up, what we do, and what must happen. The
fine-grained unit tests (366 of them, one module per data rule) are not
listed case-by-case here. They are catalogued by rule ID in
[DATA_QUALITY.md](DATA_QUALITY.md); this plan covers everything above that
layer.
...
header, and so on. Each test controls exactly what goes in and can assert
exactly what must come out. Every test also gets its own fresh, empty
SQLite database, so tests can never influence each other.
...
quarantined rows. This is the baseline. If it fails, nothing else in the
...
| Float item numbers (N2) | `"1001.0"` and `"1001.5"` | Coerce `1001.0` → `1001` (counted) and quarantine `1001.5`; half an item number means the value is corrupt, and guessing could attach data to the wrong product. |
...
| Conflicting duplicates (Q4) | the same key twice with *different* values | Load the first occurrence, archive **both** raw versions in quarantine as `conflicting_duplicate`, and raise a warning. We don't know which is true, so we keep the audit trail. |
| Mixed date formats (N3) | `23/01/2024` next to ISO dates, plus one unparseable date | Convert the day-first slash date (counted as `date_format_converted`) and quarantine the unparseable one; a *wrong* date is worse than a missing row. |
...
codes. An auditor has to be able to see exactly what was rejected and why.
...
1000; and a negative or zero limit, which would otherwise disable SQLite's
row cap entirely and dump the whole table, is rejected with `422` (three
parametrized cases: `limit=-5`, `limit=0`, `offset=-3`).

### 2.4 Upload error handling

Every one of these must fail *cleanly*: correct status code, a problem+json
body a human can act on, and no change whatsoever to previously loaded
data.
...
leaves the good data byte-for-byte intact. An ingest either fully happens
or fully doesn't.

**Replace, not merge.** Re-uploading a dataset with fewer rows leaves
exactly those fewer rows in the table. The upload *replaces* the dataset,
it never merges into it.
...
**Health.** `GET /health` answers `200 {"status": "ok"}`, which is the
check the Docker HEALTHCHECK and any scheduler relies on.

**OpenAPI snapshot.** The live OpenAPI schema must be byte-identical to the
committed `tests/snapshots/openapi.json`. Any change to the API contract, a
renamed field or a changed status code, fails this test. A contract change
is therefore always an explicit, reviewed decision rather than an accident.

---

## 4. End-to-end tests — the real challenge data (`test_real_data.py`)

This suite answers the question the whole project hangs on: given the four
real CSV files, does the service produce exactly the right result? The
expected numbers live in `tests/e2e/expected_counts.json`, generated by
`scripts/generate_expected_counts.py`, a standalone script that
re-implements the cleaning rules from the documentation **without importing
any service code**. The service and the script derive the numbers
independently; the test asserts they agree.

**Fixture sanity.** The expected-counts file must contain an entry for all
four datasets, which guards against a silently truncated regeneration.
...
Every value was traced back to the raw CSVs by hand.
...
gives a byte-identical report (ids aside), so replace semantics hold at
25k-row scale and not just on toy fixtures.

---

## 5. Container smoke test (`test_container_smoke.py`)

One test that treats the Docker image, rather than the Python code, as the
unit under test. It builds the image, starts a container on a free port
with a fresh volume, waits for `/health`, pushes all four real CSVs through
the running container, checks the reports against the same pinned counts,
...
skips with an explicit reason instead of failing. CI's `docker` job runs it
for real after `hadolint` and `docker build`.

---

## 6. What keeps this plan honest

Three meta-tests guard the suite itself. The **traceability test** pins
which functions implement which rule IDs, so this document, the code and
the tests can't silently drift apart. The **docs test** checks the ADRs and
diagrams keep their required structure. The **OpenAPI snapshot** freezes
the API contract. Coverage is enforced at ≥90% line *and* branch
