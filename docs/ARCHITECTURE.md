...
This document covers the FreshFlow service's runtime architecture, its
internal package structure and its data model, plus the two behaviors worth
understanding precisely: how ingest achieves atomic replace-per-dataset
semantics, and how tests get an isolated database without the application
code knowing it is under test. Diagrams are Mermaid. Render them with any
Mermaid-aware viewer (GitHub does it natively) or `mmdc`, or just open
...
[DATA_GUIDE.md](DATA_GUIDE.md) and [DATA_QUALITY.md](DATA_QUALITY.md). The
reasoning behind individual technology and design choices lives in the
[ADRs](adr/).
...
HTTP concerns only, meaning path/query parameters, the injected DB session,
and a call to exactly one service function. **Services orchestrate**: they
own the transaction boundary, call into the pure rule/normalization/parser
functions, and build the response and report objects. **Rules, normalize
and parser are pure**: no I/O, no database access, no FastAPI imports. They
take values in and return values, or raise `ValueError`. Models and `db.py`
sit underneath everything as the persistence layer.
...
imported by the parser, rules, models and schemas alike. It is kept
dependency-free on purpose, to avoid import cycles between `app.ingest` and
`app.schemas`.
...
no enforced foreign key, because Q2/unknown-item rows and per-store files
arriving before the catalog are both explicitly allowed (see
[ADR-004](adr/ADR-004-csv-upload-contract.md)). `ingest_jobs` and
`quarantine_rows` record ingest history and audit data.
...
report and any quarantined rows. All of it happens inside one transaction,
so a failure partway through never leaves the dataset half-updated
...
That is what "atomic replace" means concretely. There is no intermediate
state in which a client polling `GET /api/v1/stores` could observe a
...
system at all, matching [PLAN.md](PLAN.md) §3.2.

---

## 6. Layering rules, in practice

- **Routers are thin.** A router function's body accepts
  framework-validated parameters (including the injected DB session via
  `SessionDep`, see §7), calls exactly one service function, and returns
  its result — or lets a raised domain exception propagate to the handlers
  registered in `app.main.create_app`. No router file imports
...
- **Services orchestrate.** `app.ingest.service.ingest_dataset` owns the
  ...
  pure rule functions, which makes them the place where I/O and business
  logic meet.
- **Rules are pure.** `app.ingest.parser`, `app.ingest.normalize` and
  ...
  that needs a running application. That is what makes them unit-testable
  with table-driven `pytest` cases and directly traceable to rule IDs
  N1–N7/Q1–Q5 in their docstrings (see
...
`app/db.py` keeps **one module-level "current" engine and session-factory
pair** (`_engine`, `_session_factory`), built lazily from
`app.config.get_settings()` on first use via `get_engine()` /
`get_session_factory()`. Production code (`app.main.lifespan`,
`app.db.get_session`) always goes through those accessors and never
constructs an engine directly, so exactly one place decides which database
we are talking to.

Tests need a different, isolated database per test, without the
application code carrying a test-specific branch to make that happen.
`tests/conftest.py` does this two ways at once, both pointing at the *same*
isolated engine so they can't disagree:
...
1. The `engine` fixture creates a fresh SQLite database inside pytest's
   ...
   which replaces the module-level "current" engine and session factory
   for the duration of the test. Anything calling `get_engine()` or
   `get_session()` after that point gets the isolated test database, with
   no code change required in `app.db` or `app.main`. That includes
   `app.main`'s `lifespan`, which runs `init_db(get_engine())` at startup.
2. The `client` fixture also overrides the `get_session` FastAPI dependency
   directly (`app.dependency_overrides[get_session] = ...`) with a session
   factory bound to that same isolated engine. Belt and braces: the
   dependency-override mechanism is the textbook way to substitute a test
   double for `Depends(get_session)`, and binding it to the engine
   `configure_engine` already installed means the app's own
   lifespan-created engine and every request's injected session look at the
   identical database file. The app can't end up seeing a different
   database than the test's own `session` fixture.

Each test therefore gets a private SQLite file, is isolated from every
other test, and never touches the default `./freshflow.db` path — without
`app/db.py` containing a single `if TESTING` branch. This is the mechanism
as implemented in `app/db.py` and `tests/conftest.py`, not an aspiration:
`configure_engine` is a general-purpose "install a new current engine"
function used by both application startup (indirectly, via lazy
`get_engine()`) and tests (directly, in the `engine` fixture).
