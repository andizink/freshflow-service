...
carries, because part of what it demonstrates is a documented,
AI-directed build process. You should not have to read all of it to
evaluate the submission. This page is the map: what to run, what to
read, and what to skip, budgeted by the time you have.

The one principle this repo asks of you: **don't trust the prose —
verify it.** Every headline claim below is reproducible with one
command.
...
That covers: the test count (416), lint/format/`mypy --strict`
cleanliness, the ≥90% branch-coverage gate (measured ~97%), the exact
ingest table (received/loaded/deduplicated/quarantined for all four
datasets, asserted three ways: live service, pinned fixture, and an
independent stdlib-only re-derivation), and the README's sample
response for `store_a` / `2024-01-01`.
...
then read [DATA_GUIDE.md](DATA_GUIDE.md) §2, the defect catalog D1–D10,
...
- **Process**: [APPROACH.md](APPROACH.md) — how the build was directed,
  what the adversarial review caught (including in my own planning
  numbers), and the judgment calls I'd defend in person.
- **Decisions**: the [ADR index](adr/) — read ADR-003 (clean at ingest),
...
| [DATA_GUIDE.md](DATA_GUIDE.md) | Defect catalog D1–D10 + what the pipeline does per defect | 30-min pass |
| [APPROACH.md](APPROACH.md) | How the build was directed; what review caught | AI-process assessment |
| [ADRs](adr/) | Each non-obvious decision with alternatives | As needed per decision |

**Supporting** (deepen specific angles):

| Doc | What it is |
|---|---|
| [ARCHITECTURE.md](ARCHITECTURE.md) | Runtime/component/ER diagrams, sequences |
| [DATA_QUALITY.md](DATA_QUALITY.md) | Formal defect → rule → code → test traceability matrix |
| [TEST_PLAN.md](TEST_PLAN.md) | Plain-language description of integration/e2e cases |
| [PROBLEM_ANALYSIS.md](PROBLEM_ANALYSIS.md) | Domain framing for readers new to grocery ordering |
...
| Doc | What it is |
|---|---|
| [PLAN.md](PLAN.md) | The frozen pre-implementation plan the agent team executed |
| [KICKOFF_PROMPT.md](KICKOFF_PROMPT.md) | The verbatim prompt that launched the build |
